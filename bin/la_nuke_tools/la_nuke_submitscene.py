...
try:
    print '#' * 80
    print ''
    dry_run = raw_input('Is this a dry run? [y/n] : ')
    if 'y' in dry_run.lower():
        dry_run = True
    else:
        dry_run = False
    ...
    ######################################################################################################################
        ...
        scene_choice = int(raw_input('\tPlease enter the scene you want to submit? : '))

        print '\tFinding all JSON files...'
        scene = 'sc%03d' % scene_choice
        list_of_json_files = glob.glob( os.path.join(shots_root, 'act*', scene, 'sh*', 'shot_info.json') )
        list_of_shots = []
        if list_of_json_files:
            print '\t\tFound %04d entries.' % len(list_of_json_files)
            list_of_json_files.sort()
