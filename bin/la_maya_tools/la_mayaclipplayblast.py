...
if renderCamera != None:
    ...
    cmd += ' -s 2048x1152'
    cmd += ' -filter_complex "[0:v]eq=gamma=2.2[G];[G]drawtext=\'fontcolor=white:font=sans-serif:fontsize=12:x=6:y=555:text=  luma-film - 2020 - inside job %s - %s - %s - frame %%{frame_num}:box=1:boxborderw=5:boxcolor=black:start_number=%d\'[LT]"' % (date, shot_name, user, frame_s)
